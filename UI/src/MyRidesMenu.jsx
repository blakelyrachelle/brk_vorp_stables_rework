...
function MyRidesMenu({ Data: { characterId, rides } }) {
  ...
  useEffect(() => {
    ...
    const previewTimer = setTimeout(() => {
      ...
      axios.post(`https://${GetParentResourceName()}/activateCam`, {
        rideType: ride.type
      }).catch(() => {});
  ...
  }, [currentRideIndex, modalOn, rides]);

  function setDefault() {
    axios.post(`https://${GetParentResourceName()}/setDefault`, {
      newRide: ride.id,
      prevRide: rides?.find(r => r.type === ride.type && r.isDefault)?.id
    }).catch(() => {});
  }
            ...
            <span>
              <button onClick={() => {
                ...
                if (confirm.toLowerCase() === ride.name.toLowerCase()) {
                  axios.post(`https://${GetParentResourceName()}/free`, {
                    rideId: ride.id
                  }).catch(() => {});
                }
                setModalOn(false);
      ...
      <menu>
        {!ride.isDefault && <span onClick={setDefault}>{Data.Lang.ChooseAsActive.replace("{rideType}", Data.Lang[ride.type])}</span>}
        {
          ...
          <>
            ...
            <span onClick={() => setRoute("/tackstorage?" + ride.id)}>Stored Tack</span>
            <span onClick={() => axios.post(`https://${GetParentResourceName()}/removeAllComps`, { rideId: ride.id }).catch(() => {})}>{Data.Lang.RemoveEquipment}</span>
          </>
        }
