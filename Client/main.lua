...
local stableLoadRequested = false
local vendorHandles = {}
local blipHandles = {}
    ...
    if stableInRange == nil then
        for index, stable in pairs(Config.Stables) do
            ...
            if inDistance(pCoords, sCoords, radius) then
                ...
                if not Stable and not stableLoadRequested then
                    stableLoadRequested = true
        ...
        elseif not IsNuiFocused() then
            ...
            if UiPromptHasHoldModeCompleted(PromptOpenVendor) then
                if not Stable or not Stable.rides then
                    if not stableLoadRequested then
                        stableLoadRequested = true
                        TriggerServerEvent(Events.loadStableRuntime)
                    end

                    TriggerEvent("vorp:TipRight", "Stable data is still loading. Try again.", 4000)
                else
                    StableMenuOpen = true
...
RegisterNetEvent(Events.onStableLoaded, function(result)
    stableLoadRequested = false
    Stable = PlayerStable:new(
    result.rides or {},
    result.charId,
    result.availableComps or {},
    result.transferedRides or {},
    result.tackStorage or {},
    result.tackLoadout or {}
)
