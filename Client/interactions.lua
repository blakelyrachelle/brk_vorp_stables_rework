...
local function GetCompHash(comp)
    if type(comp) == "table" then--more new hit dont forget
        comp = comp.hash or comp.comp_hash or comp.model or comp[1]
            ...
            if type(comp) == "string" and comp:sub(1, 2):lower() == "0x" then
                return tonumber(comp:sub(3), 16)
            end
        return tonumber(comp)
    end

local function ApplySavedCompToHorse(entity, comp)
...
local function finishHorseSpawn(ride)
    ...
    local ped = PlayerId()
        ...
        for _, comp in pairs(ride.comps or {}) do
            ApplySavedCompToHorse(ride.pedId, comp)
        end
    UpdatePedVariation(ride.pedId)
